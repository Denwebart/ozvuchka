...
<div class="row">
    <div class="col-sm-12 text-xs-center">
        <a href="#" class="button-create-gallery-image btn btn-inverse waves-effect waves-light m-b-20 pull-right" data-url="{{ route('admin.gallery.create') }}" data-toggle="modal" data-target="#gallery-modal" data-modal-id="gallery-modal" data-animation="fadein" data-overlaySpeed="200" data-overlayColor="#36404a">
            <i class="md md-add"></i>
...
<div class="row">
    <div id="table-container">
        @include('admin::gallery._table')
    </div>
</div>
<!-- end row -->

<!-- Responsive modal for creating and editing gallery images -->
<div id="gallery-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var gallerySortableOptions = {
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: '{{ route('admin.gallery.position') }}',
        ...
        $(".sortable-gallery-images").sortable(gallerySortableOptions);

        // Init plugins after ajax
        function initPluginsAfterAjax() {
            $(".sortable-gallery-images").sortable(gallerySortableOptions);
            $('#table-container').find('[data-plugin="switchery"]').each(function (i, o) {
        ...
        // Creating/editing gallery image: open popup form
        $(document).on('click', '.button-create-gallery-image, .button-edit', function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        $('#gallery-modal .ajax-modal-content').show().html(response.resultHtml);
                        $('#gallery-modal .loader').hide();
                        // Bootstrap MaxLength
                        $(".maxlength").maxlength({
                            alwaysShow: true
                        });
        ...
        // Creating/editing gallery image: ajax request
        $('#gallery-modal').on('click', '.button-save', function(e) {
            e.preventDefault ? e.preventDefault() : e.returnValue = false;
...
//                headers: {"X-HTTP-Method-Override": "PUT"},
                ...
                success: function (response) {
                    if (response.success) {
                        $('#gallery-modal').find('.close').trigger('click');
                        $('#gallery-modal .ajax-modal-content').hide();
                        ...
                        $('#gallery-modal .loader').hide();
                        $('#gallery-modal .ajax-modal-content').show();
